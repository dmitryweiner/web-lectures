...
### Авторизация
* Для авторизации обычно используются куки. В них кладут сессионный токен.
* У куки должны быть следующий параметры ([подробнее](https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies)):
  ...
  * sameSite=lax: чтобы можно было обращаться с другого домена (если это необходимо).
...
### Проверка состояния авторизованности
```js
router.get('/', (req, res) => {
